...
#function 1: pull the initial data from the api and store the close prices in a csv file (coingecko api only provides close prices)
def initial_data_pull(coins):
    for coin in coins:
        ...
        close_prices = [f'{coin}, {datetime.fromtimestamp(timestamp/1000, tz=timezone.utc):%Y-%m-%d}, {value}\n'
                    for timestamp, value in data]
        
        #store the reserverd close_prices list in a csv file
        with open("Final_Project/cryptocurrency_trading/coingecko/" + coin + "_close_prices.csv", "w") as f:
            f.writelines(close_prices)

        #wait for 30 seconds before pulling the next coin's data
        time.sleep(30)
...
def append_data(coins):
    ...
    for coin in coins:
        ...
        with open("Final_Project/cryptocurrency_trading/coingecko/" + coin + "_close_prices.csv", "r") as f:
            last_day_csv = f.readlines()[-1].split(",")[1].strip()

        #create a list to store the close prices need to be appended
        close_prices_to_append = []

        #iterate through the data and extract the close prices to append
        for timestamp, value in data[::-1]:
            ...
            if date == last_day_csv:
                ...
                close_prices_to_append.append(f'{coin}, {date}, {value}\n')

        #append the close_prices_to_append to the csv file
        with open("Final_Project/cryptocurrency_trading/coingecko/" + coin + "_close_prices.csv", "a") as f:
            f.writelines(close_prices_to_append[::-1])
...
def check_last_day_signal(prices,n):
    ...
    if last_price < mean*0.98:
        signals.append(f"based on the mean reversion strategy: you should BUY {coin} coin tomorrow")
    elif last_price > mean*1.02:
        signals.append(f"based on the mean reversion strategy: you should SELL {coin} coin tomorrow")

    #simple moving average strategy signal
    if last_price > mean:
        signals.append(f"based on the simple moving average strategy: you should BUY {coin} coin tomorrow")
    elif last_price < mean:
        signals.append(f"based on the simple moving average strategy: you should SELL {coin} coin tomorrow")

    #bollinger bands strategy signal
    if last_price < mean*0.95:
        signals.append(f"based on the bollinger bands strategy: you should BUY {coin} coin tomorrow")
    elif last_price > mean*1.05:
        signals.append(f"based on the bollinger bands strategy: you should SELL {coin} coin tomorrow")
...
def SaveResults(dictionary):
    json.dump(dictionary,open('Final_Project/cryptocurrency_trading/coingecko/results.json','w'),indent=4)
...
for coin in coins:
    ...
    prices = [float(line.split(",")[2].strip()) for line in reversed(list(open('Final_Project/cryptocurrency_trading/coingecko/' + coin + '_close_prices.csv','r')))]
